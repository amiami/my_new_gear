...
公開ページには写真の向きに合わせた1200×630pxのOGP画像が生成されます。Xへの共有は
ガジェット名やコメントを投稿文へ含めず、定型文・ハッシュタグ・実際の公開URLを編集可能な
投稿画面へ渡します。公開解除後はOGP画像も取得できなくなりますが、Xなど外部サービスが
すでに取得したキャッシュの即時削除は保証されません。
